...
fun TripDetailScreen(tripId: String?, navController: NavController) {
    val trip = MockData.tripsList.find { it.id.toString() == tripId }
                    ...
                    confirmButton = {
                        TextButton(onClick = {
                            if (newPlaceName.isNotBlank() && trip != null) {
                                MockData.addNewPlaceToTrip(trip.id, newPlaceName)
                ...
                items(trip.places) { place ->
                    PlaceItem(
                        ...
                        onCheckedChange = { isChecked ->
                            val tripIndex = MockData.tripsList.indexOfFirst { it.id == trip.id }
                            if (tripIndex != -1) {
                                ...
                                MockData.tripsList[tripIndex] = trip.copy(
